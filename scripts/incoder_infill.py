...
def apr_infill(text, max_to_generate=128, temperature=0.2):
    parts = text.split('<INFILL>')
    # "p=0.95" has been hard-coded into scripts
    result = infill(parts, max_to_generate=max_to_generate, temperature=temperature)
    # print("completed document:")
    # print(result["text"])
    return result["text"]
...
if __name__ == "__main__":
    ...
    # docstring_to_code()
    with open('/data2/zhijietang/temp/llm_infill_temp/gson_1_prompts.json', 'r') as f:
        prompts = json.load(f)
    res = apr_infill(prompts[0]['func_infill_prompts'][0]['func_prompt'], 512)
